...
<Steps>
  ...
  <Step title="Add Cartesia Account Details">
    ...
    <Frame caption="Cartesia Model Account Form">
      <img src="./assets/images/truefoundry/cartesia2.avif" alt="Cartesia account configuration form with fields for API key and collaborators" />
    </Frame>
  </Step>
  ...
  <Step title="Add Models">
    ...
    <Frame caption="Add Cartesia Model">
      <img src="./assets/images/truefoundry/cartesia3.avif" alt="Add Cartesia model form with display name, model ID, and model type" />
    </Frame>
...
async def main():
    with open(AUDIO_FILE, "rb") as f:
        response = await client.stt.transcribe(
            file=f,
            model="ink-whisper",  # must match a model ID registered in AI Gateway
            language="en",
        )
    print(response.text)
