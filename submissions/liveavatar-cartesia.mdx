...
description: "Drive a LiveAvatar interactive avatar with a Cartesia Sonic TTS voice"
...
Use this integration to speak a [LiveAvatar](https://docs.liveavatar.com) avatar's responses with a [Cartesia Sonic TTS](/build-with-cartesia/tts-models/latest) voice. LiveAvatar handles the LLM, streaming video, and lip sync; Cartesia generates the audio.

Set `provider` to `cartesia` in the session's [voice settings](https://docs.liveavatar.com/docs/full-mode/voice-settings) to synthesize the selected LiveAvatar voice with Sonic. You can also bring your own Cartesia API key and import your own Cartesia voices — see [Use your own Cartesia voices](#use-your-own-cartesia-voices).
...
<Steps>
  ...
  <Step title="Set Cartesia as the TTS provider" titleSize="h3">
    Add `voice_settings` to `avatar_persona` in `apps/demo/app/api/start-session/route.ts`. The `provider` field selects which TTS engine renders the voice:
  ...
  <Step title="Run the demo" titleSize="h3">
    ...
    Open [http://localhost:3001](http://localhost:3001), click **Full Mode**, then unmute and talk. The avatar answers in the Sonic voice.
  </Step>
</Steps>
...
The response contains a `secret_id`. Pass it with the Cartesia voice ID to create a LiveAvatar voice:
...
The response returns a LiveAvatar `voice_id`. Use it as `VOICE_ID` in `secrets.ts`. LiveAvatar generates that session's audio with your Cartesia credentials.
