...
<Check>Last verified: 2026-06-03</Check>

## Overview

Add live web search and page extraction to a [Cartesia Line](/line/sdk/agents) voice agent using [loopback tools](/line/sdk/tools) backed by [Tavily](https://docs.tavily.com). The agent can answer questions about current events, fresh facts, and specific URLs mid-call.

## Prerequisites

- Python 3.10+
- A Cartesia API key (`CARTESIA_API_KEY`) from [Cartesia keys](https://play.cartesia.ai/keys)
- A Tavily API key (`TAVILY_API_KEY`) from the [Tavily dashboard](https://app.tavily.com/home)
...
Define two [loopback tools](/line/sdk/tools) backed by `AsyncTavilyClient` — one for search, one for full-page extraction — and pass them to an `LlmAgent`. Run the file, then connect with `cartesia chat 8000`.
...
See the [Tavily Search API reference](https://docs.tavily.com/documentation/api-reference/endpoint/search) for the full parameter set, including `topic`, `include_domains`, and `country`.

## Resources

- [Runnable example in the Cartesia Line repo](https://github.com/cartesia-ai/line/tree/main/example_integrations/tavily)
