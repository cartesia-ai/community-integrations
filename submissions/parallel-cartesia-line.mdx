...
Use [Parallel Search](https://docs.parallel.ai/search/search-quickstart) as a [loopback tool](/line/sdk/tools) in a [Cartesia Line agent](/line/sdk/agents). This example uses `mode="turbo"` for a foreground voice turn, where the agent needs current web context before answering in short spoken sentences.
...
<Steps>
  ...
  <Step title="Ask a current-information question" titleSize="h3">
    ```text
    What is the latest news about Parallel Web Systems compensating content owners?
    ```
  </Step>
</Steps>
...
The defaults keep search results and excerpts compact so the agent gets enough context for a spoken answer without turning the call into a deeper research workflow.
