...
Add Parallel Search to a [Cartesia Line](/line/sdk/agents) voice agent with a [loopback tool](/line/sdk/tools). The agent searches during the voice turn and answers with brief spoken responses.
...
def format_for_voice(results: list) -> str:
    ...
    for index, result in enumerate(results[:MAX_RESULTS], start=1):
        ...
        result_lines.append(
            f"Result {index}.{published} Excerpt: {squash(excerpts[0], 360)}"
        )
