...
description: "Run a two-way voice agent on a Bandwidth phone call with Cartesia Ink speech-to-text and Sonic text-to-speech"
...
```
   ...
   │        │  decode ──► Ink STT  (raw μ-law)  ── transcript ─┐ │
...
<Steps>
  ...
  <Step title="Write the bridge server" titleSize="h3">
    Create `server.py`. It answers Bandwidth's webhook with a `<StartStream>` verb, opens the media WebSocket, forwards caller audio to Ink, and speaks each finalized transcript back through Sonic.
    ...
    # Ink reads the call's native 8 kHz mu-law directly, so caller audio needs no resampling.
    STT_URL = (
    ...
    async def stream(ws: WebSocket) -> None:
        ...
        # One task pumps caller audio into Ink; the other speaks Ink's transcripts back.
    ...
    async def _caller_audio_to_stt(ws: WebSocket, stt) -> None:
        # Bandwidth sends each media event as base64 mu-law in JSON; Ink wants raw
        ...
        try:
            ...
            # Flush Ink's buffered audio and close its session cleanly.
...
- [Ink STT models](/build-with-cartesia/stt/latest)
