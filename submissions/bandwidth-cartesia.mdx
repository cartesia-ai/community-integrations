...
- **Cleaner turn-taking.** The manual STT socket emits incremental `is_final` segments, so this demo replies per fragment. Switch to the [turn-detection endpoint](/api-reference/stt/websocket) (`/stt/turns/websocket`) to reply once per completed utterance.
